...
def phase_tap_changer_query(
    ...
    where_list.extend(
        [
            f"bind(?pt as ?w_mrid_1)",
            f"bind(?pt as ?w_mrid_2)",
            f"bind(?term_1 as ?t_mrid_1)",
            f"bind(?term_1 as ?t_mrid_2)",
        ]
    )
...
def transformers_connected_to_converter(
    ...
    converters = [
        sup.rdf_type_tripler("?volt", f"cim:{converter}") for converter in converter_types
    ]
    where_sub_list = [
        sup.rdf_type_tripler(mrid, "cim:PowerTransformer"),
